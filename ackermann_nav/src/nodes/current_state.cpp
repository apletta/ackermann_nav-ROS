...
public:
  ...
  pub_1 = n.advertise<myStateMsg>("state_vehicle", 1000);
  pub_2 = n.advertise<myPositionMsg>("curr_position", 1000);

  //set subscriber topics and queue size for subscribed messages
  sub_1.subscribe(n, "state_odom", 1000);
  sub_2.subscribe(n, "state_gps", 1000);
  sub_3.subscribe(n, "state_imu", 1000);
  ...
  void callback(const myStateMsg::ConstPtr& sub_1_info, const myStateMsg::ConstPtr& sub_2_info, const myStateMsg::ConstPtr& sub_3_info)
  {
    //create message object(s), need one per topic you want to publish to
    myStateMsg output;
    myPositionMsg output2;

    //Read in subscribed info to variables
    double sub_1_info_vel_k = sub_1_info->vel_k;
    double sub_1_info_head_k = sub_1_info->head_k;
    double sub_1_info_x_k = sub_1_info->x_k;
    double sub_1_info_y_k = sub_1_info->y_k;   

    double sub_2_info_vel_k = sub_2_info->vel_k;
    double sub_2_info_head_k = sub_2_info->head_k;
    ...
    */
    ...
    output.vel_k = sub_1_info_vel_k;  //this line can be set to your variable
    output.head_k = sub_1_info_head_k;  //this line can be set to your variable
    output.x_k = sub_1_info_x_k;
    output.y_k = sub_1_info_y_k;


    output2.header.stamp = ros::Time::now();  //this line likely won't change
    output2.x_k = sub_1_info_x_k;  //this line can be set to your variable
    output2.y_k = sub_1_info_y_k;  //this line can be set to your variable
...
private:
  ...
  message_filters::Subscriber<myStateMsg> sub_1;
  message_filters::Subscriber<myStateMsg> sub_2;
  message_filters::Subscriber<myStateMsg> sub_3;
  

  //define sync policy as ApproximateTime, one type per topic to subscribe to
  typedef sync_policies::ApproximateTime<myStateMsg, myStateMsg, myStateMsg> MySyncPolicy;
