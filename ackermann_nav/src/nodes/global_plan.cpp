...
public:
  ...
  pub_1 = n.advertise<myCheckpointMsg>("checkpoints", 1000);

  //set subscriber topics and queue size for subscribed messages
  sub_1.subscribe(n, "goal", 1000);
  ...
  void callback(const myGoalMsg::ConstPtr& sub_1_info, const myPositionMsg::ConstPtr& sub_2_info)
  {
    //create message object(s), need one per topic you want to publish to
    myCheckpointMsg output;

    //Read in subscribed info to variables
    double sub_1_info_goal_x = sub_1_info->goal_x;
    double sub_1_info_goal_y = sub_1_info->goal_y;   
    ...
    */
    ...
    output.ckpt_x = sub_1_info_obs_x;  //this line can be set to your variable
    output.ckpt_y = sub_1_info_obs_y;  //this line can be set to your variable
